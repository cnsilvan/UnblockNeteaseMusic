...
func bitTransform(array []int, len int, bts *big.Int) *big.Int {
	...
	for bti = 0; bti < len; bti++ {
		if array[bti] >= 0 && new(big.Int).And(bts, gArrayMask[array[bti]]).Uint64() != 0 {
...
func desSubKeys(key *big.Int, K []*big.Int, mode int) {
	...
	for j = 0; j < 16; j++ {
		...
		{
			source := temp
			mask := new(big.Int).SetUint64(gArrayLsMask[gArrayLs[j]])
			left := new(big.Int).And(source, mask)
			left = left.Lsh(left, uint(28-gArrayLs[j]))
			right := new(big.Int).And(source, mask.Not(mask))
			right = right.Rsh(right, uint(gArrayLs[j]))
			temp = left.Or(left, right)
		}
		/* PC-2变换 */
		// 要初始化k的元素为0
...
func des64(subkeys []*big.Int, data *big.Int) *big.Int {
	...
	pSource[0] = new(big.Int).And(out, new(big.Int).SetUint64(0x00000000ffffffff))
	pSource[1] = new(big.Int).And(out, new(big.Int).SetInt64(-4294967296))
	pSource[1] = pSource[1].Rsh(pSource[1], 32)
	/* 主迭代 */
	// source = out;
	for i := 0; i < 16; i++ {
		/* F变换开始 */
		R = new(big.Int).Set(pSource[1])
		/* E变换 */
		...
		/* S盒变换 */
		for k := 0; k < 8; k++ {
			kk := new(big.Int).Rsh(R, uint(k*8))
			pR[k] = kk.And(kk, new(big.Int).SetUint64(0xff))
		}
		SOut = SOut.SetUint64(0)
		for sbi = 7; sbi >= 0; sbi-- {
			SOut = SOut.Lsh(SOut, 4).Or(SOut, new(big.Int).SetInt64(int64(gMatrixnsBox[sbi][pR[sbi].Uint64()])))
		}
		//R = SOut
		/* P变换 */
		R = bitTransform(gArrayP, 32, SOut)
		/* f变换完成 */
		L = new(big.Int).Set(pSource[0])
		pSource[0] = new(big.Int).Set(pSource[1])
		pSource[1] = pSource[1].Xor(L, R)
	}
	/* 交换高低32位 */
	t = new(big.Int).Set(pSource[0])
	pSource[0] = pSource[1]
	pSource[1] = t
	/* IP-1变换 */
	out = out.Lsh(pSource[1], 32).And(out, new(big.Int).SetInt64(-4294967296)).Or(out, pSource[0].And(pSource[0], new(big.Int).SetInt64(0x00000000ffffffff)))
	out = bitTransform(gArrayIP1, 64, out)
	return out
}
func Encrypt(src []byte) []byte {
	return encrypt(src, SECRET_KEY)
}

func encrypt(src []byte, key []byte) []byte {
	// long keyl = Long.valueOf(new String(key));

	var keyl = new(big.Int).SetInt64(0)
	srcLength := len(src)
	for i := 0; i < 8; i++ {
		kB := new(big.Int).SetBytes([]byte{key[i]})
	...
	temp := 0
	ff:=new(big.Int).SetInt64(255)
	// 将密文转为字节型
	for i := 0; i < len(pEncyrpt); i++ {
		for j := 0; j < 8; j++ {
			enc:=new(big.Int).Set(pEncyrpt[i])
